package com.vn.vbeeon.presentation.adapter.deviceAddNew
...
class DeviceAdapter(
    val doneClick: (Device) -> Unit,
    val onClickDetail: (item: Device) -> Unit
) : RecyclerView.Adapter<DeviceAdapter.ItemViewHoder>() {
    var frames: List<Device> = mutableListOf()

    inner class ItemViewHoder(view: View) : RecyclerView.ViewHolder(view) {
        init {
            itemView.setOnClickListener {
                doneClick(frames[layoutPosition])
            }
            itemView.tvDetail.setOnSafeClickListener {
                onClickDetail(frames[layoutPosition])
            }
        }

        fun bindData(device: Device) {
            itemView.tvDeviceCategoryNew.text = device.categoryName
            itemView.tvDeviceNameNew.text = device.name
            if (device.intSource != 0) {
                Glide.with(itemView.context).load(device.intSource).into(itemView.imgDeviceNew)
            } else
                Glide.with(itemView.context).load(device.intSource).into(itemView.imgDeviceNew)
        }
    }
...
//    }
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ItemViewHoder {
        val holder =
            LayoutInflater.from(parent.context).inflate(R.layout.item_device_new, parent, false)
